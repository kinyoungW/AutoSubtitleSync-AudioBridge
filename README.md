...
2. 装浏览器插件 **Tampermonkey**（推荐用 Edge，加载项商店国内可直接访问），
   然后把 `BrowserAudioBridge.user.js` **拖进浏览器窗口**完成脚本安装。
   **别去点面板里的安装链接**——Chromium 已不允许从网页装用户脚本，会报
   "Apps, extensions, and user scripts cannot be added from this website"。
   完整步骤与排查见 [`实时字幕（音频桥）使用说明.md`](实时字幕（音频桥）使用说明.md) 第一节；
...
| 来源 | 能抓到什么 | 需要准备 |
| --- | --- | --- |
| **播放器元素**（默认） | 页面里 `<video>/<audio>` 的声音；唯一支持「影子播放器」前瞻 | 无 |
| **共享标签页音频** | 这个标签页最终输出的声音——iframe / MSE / blob / 跨域 / WebAudio 全通吃 | 点一次浏览器弹窗勾选「分享标签页音频」 |
| **系统声音** | 电脑正在播放的任何声音（含 DRM） | 装一次 BlackHole 虚拟声卡（见使用说明第三节） |

播放器藏在 iframe 里也不用管：脚本在每个框架里都跑，会自动把抓音交给有播放器的那一帧，字幕叠在播放器上。
